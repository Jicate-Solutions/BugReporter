...
export async function runDispatcher(admin: SupabaseClient): Promise<DispatchSummary> {
  ...
  // ── 1. FIRE ────────────────────────────────────────────────────────────────
  const { data: due, error: dueErr } = await admin.rpc('fn_app_routine_claim_due');
  ...
  for (const r of routines) {
    ...
    try {
      ...
      await recordRun(admin, r, 'running', jobId, null, null);
      summary.enqueued++;
    } catch (e) {
  ...
  if (cfg) {
    const { data: pending } = await admin
      ...
      .select('id, organization_id, job_id, routine_kind')
...
async function recordRun(
  ...
  await admin.from('app_ai_routine_runs').insert({
    routine_id: r.id,
